...
\midrule
\textbf{Total (target)}       & $\mathbf{1450}$ & \textbf{Design target} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Edge                       & $0.251$ & $2.85$ \\
\textbf{Corner (critical)} & $\mathbf{0.277}$ & $\mathbf{3.15}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
OD & Nuts & Wheel mass & Wheel $I_w$ & $I_w$/target & 3 wheels & Pocket fit \\
(mm) & $N$ & (g) & ($10^{-4}$ kg\,m$^2$) & (\%) & (g) & \\
\midrule
...
\textbf{120} & \textbf{21} & \textbf{131.5} & \textbf{3.31} & \textbf{105.2} & \textbf{394.6} & \textbf{OK ($\sim$2.5 mm)} \\
...
The full parts list is catalogued in Appendix~\ref{app:bom}: the supplied
components grouped by subsystem (Table~\ref{tab:bom-supplied}), the actuation
figures derived from them (Table~\ref{tab:bom-derived}), and the mechanical and
consumable items still to be procured (Table~\ref{tab:bom-gaps}). The supplied
set is an electronics-only BOM; the reaction-wheel rims, the frame and all
printed parts are the team's design responsibility, and their procurement is the
leading schedule risk (Table~\ref{tab:bom-gaps}).
