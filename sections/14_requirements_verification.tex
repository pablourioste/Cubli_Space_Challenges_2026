...
FUN-07 & D & Exercise every transition of the supervisory state machine required by FUN-07 (Table~\ref{tab:requirements}), including at each threshold boundary. & All transitions taken as specified; no chattering at any boundary. & Open \\
...
PER-12 & A, T & Compute $t_{\text{spin}} = I_w\omega_{\max}/k_ti$ from the $K_V$-derived torque constant; confirm on the built wheel by logging speed against time through a spin-up at the continuous current limit (WBS E10). & Design speed reached within \qty{2}{\second} without exceeding \qty{9}{\ampere}. & Open (analysis: \qty{\tspin}{\second}) \\
PER-13 & A, T & Compare the commanded spin-up torque against $\tau_g=MgL/2$ at the as-built mass; observe the body on a face rest through a full spin-up. & Reaction torque $\leq 0.5\,\tau_g$; no body motion before the brake event. & Open (analysis: \qty{\pcDriveGrav}{\percent}) \\
\midrule
...
CON-02 & I & Weigh the integrated cube, ready to run. & $\leq$ \qty{1.8}{\kilo\gram}. & \textbf{Verified} (\qty{1.626}{\kilo\gram}, App.~\ref{app:cad_mass}) \\
