...
follows to take \projectname{} from a sizing target to a validated prototype, why
its six stages run in this order and not another, and why the result is an
iteration rather than a single pass through them. \projectname{} is an
...
\item \textbf{Preliminary Sizing} is the first stage of the project: it derives
  the physics behind \projectname{} and justifies the design parameters and
...
\item \textbf{Firmware Architecture} is developed in parallel with the
  ...
  (Section~\ref{sec:firmware-interface}).
...
Fully assembled, \projectname{} is underactuated and simultaneously unstable
about all three axes when balanced on a corner (Section~\ref{sec:objectives}). Stage~3 of the
