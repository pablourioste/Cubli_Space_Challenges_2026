...
\paragraph{Design drivers.} No single subsystem is exotic, but each sits close to a physical bound, and the bounds are coupled: relieving one tightens another. Four constraints drive the design, and sizing is therefore treated here as the first engineering activity rather than a detail settled once parts arrive.
...
\paragraph{Why a low-cost build.} The nominal Cubli problem was solved a decade ago on precisely manufactured hardware \cite{gajamohan2012cubli, gajamohan2013cubli, muehlebach2017cubli}; reproducing it is an engineering exercise, and this document claims no more. The value of a commodity build is that it can be crashed repeatedly, so the controller can be tested at its limits, and that loose tolerances, an imperfectly balanced wheel and a drifting IMU supply exactly the parameter uncertainty that robust control methods claim to tolerate. A fixed motor, print volume and budget also force honest sizing: a marginal design cannot be rescued by specifying a larger component, which is the justification for the adjustable ballast scheme of Section~\ref{sec:rw_sizing}.
