...
\begin{minipage}[t]{0.32\textwidth}
  ...
  {\small (c) SpaceHopper}
\end{minipage}
\caption[Flight and research platforms using momentum-exchange mobility]{Momentum-exchange actuation in flight and research hardware, the
mechanism the Cubli demonstrates terrestrially. (a) JAXA's MINERVA-II1 rovers
\cite{yoshimitsu1999minerva} and (b) the MASCOT lander \cite{ho2017mascot},
both deployed by Hayabusa2 onto asteroid Ryugu in 2018, hop across the surface
by spinning up and braking an internal flywheel or eccentric mass; MASCOT
self-rights the same way. (c) ETH Zurich's SpaceHopper
\cite{spiridonov2024spacehopper} extends the principle to controlled,
directional hopping in low gravity --- the research frontier that the walking
objective of Section~\ref{sec:objectives} targets.}
...
\begin{enumerate}
    \item \textbf{Edge balance} — stabilise the cube on a single edge, the least unstable equilibrium (contact is a line, one primary axis of instability).
    \item \textbf{Corner balance} — stabilise the cube on a single vertex, the fully unstable equilibrium (point contact, all three axes coupled and unstable simultaneously).
    ...
    \item \textbf{Walking} — chain repeated, directional controlled falls between edges/corners into a net translation across a surface, re-stabilising after each transition.
\end{enumerate}
