...
alignment (Section~\ref{sec:layout}). Every raw gyro/accel sample is therefore
rotated into $\{B\}$ by a fixed mounting matrix before it reaches the
complementary filter of Equation~\eqref{eq:complementary} or the MEKF of
Section~\ref{sec:software_pipeline}.
...
diagonal from $O$. $C_{SB}$ is verified orthonormal (unit determinant, unit
row norms) whenever the mounting angles are set, guarding against a
