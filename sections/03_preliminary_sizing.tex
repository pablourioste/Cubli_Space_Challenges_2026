...
(\texttt{analysis/SIZING.py}): $M$ is a fixed point of the closure, not an
assumption, reproduced by the Stage~6 roll-up to within \qty{0.1}{\gram}.
$\tau_b$ is an assumed value pending measurement, justified below and
equivalent to a \qty{52}{\milli\second} stop at the present $h_w$.}
\label{tab:jumpup_budget}
\begin{tabular}{llr}
\toprule
Symbol & Description & Value \\
\midrule
$M$                & Cube mass                    & \qty{1.772}{\kilo\gram} \\
...
$h_w$              & Required angular momentum    & \qty{0.2588}{\kilo\gram\metre\squared\per\second} \\
$I_{w,\text{target}}$ & \textbf{Target wheel inertia} & \qty{4.119e-4}{\kilogram\metre\squared} \\
\bottomrule
\end{tabular}
...
material, checked in Section~\ref{sec:sizing_verification}, and a nyloc
...
\midrule
Required wheel speed $\omega_w$ & Collision relation, Sec.~\ref{sec:jumpup} & \qty{5374}{\rpm} & $\leq$ achievable speed with margin \\
Achievable wheel speed $\omega_{\max}$ & Drive analysis, Sec.~\ref{sec:jumpup_target} & \qty{6000}{\rpm} & Sustained in service, not no-load \\
Speed margin $\omega_w/\omega_{\max}$ & Derived & \num{0.896} & $<1$ by a stated margin \\
Target inertia $I_{w,\text{target}}$ & Eq.~\eqref{eq:Iw_target} & \qty{4.119e-4}{\kilogram\metre\squared} & --- \\
Achieved inertia $I_w$ & Wheel design, Sec.~\ref{sec:rw_sizing} & \qty{4.599e-4}{\kilogram\metre\squared} & $\geq I_{w,\text{target}}$ \\
Route disagreement & Both of the above & \qty{+11.7}{\percent} & Reported; explained if large \\
Ballast trim authority & Sec.~\ref{sec:rw_sizing} & $-1 / +24$ stations & Non-zero in both directions \\
Ballast retention margin & Sec.~\ref{sec:rw_sizing} & \qty{1.47}{\percent} of proof; \qty{4.02}{\mega\pascal} bearing & Bolt tension and head/nut bearing stress within allowables \\
\bottomrule
\end{tabular}
\end{table}

The gate passes on every row, but the trim authority is markedly asymmetric and
that asymmetry is the design's tightest margin. Upward trim is ample: the gap
rule admits \num{24} further stations, enough to more than double the inertia.
Downward trim is nearly exhausted, because the wheel sits only
\qty{11.7}{\percent} above target while each station is worth
\qty{5.9}{\percent} of it --- removing two stations already falls below
requirement. The practical consequence is that this design absorbs a
\emph{heavier}-than-predicted cube comfortably and a lighter one hardly at all,
which is the favourable direction given that \qty{34}{\percent} of
Table~\ref{tab:massbudget} is still allowance and allowances more often prove
optimistic than pessimistic. Should the structure come in light, the relief
variable is $\omega_{\max}$ rather than the ballast count.
