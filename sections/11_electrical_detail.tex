...
\textcolor{red}{TBD} & $R_{\text{term}}$ (end A, end B) & --- & \qty{120}{\ohm} each & Termination, two physical ends only; \qty{60}{\ohm} $\pm$\qty{3}{\ohm} unpowered across CAN\_H--CAN\_L (Sec.~\ref{sec:electrical}) \\
\bottomrule
\end{tabular}
\end{table}
...
J10-3 & 5V\_LOGIC & Out (board$\to$ESP32) & \qty{5.00}{\volt} & ESP32 supply, sized for the \qty{300}{\milli\ampere} transmit burst \\
...
J11-3 & GND & In/Out & 0\,V & Servo return; flyback path verified during electrical bring-up (Sec.~\ref{sec:electrical}) \\
\bottomrule
\end{tabular}
\end{table}
% ---------------------------------------------------------------------------
