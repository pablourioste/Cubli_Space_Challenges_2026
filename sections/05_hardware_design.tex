...
a common centre, carries the three motor drivers, the microcontroller, the IMU
...
\midrule
Edge                       & $0.1434$ & $0.2236$ \\
\textbf{Corner (critical)} & $2.28\times10^{-4}$ & $3.56\times10^{-4}$ \\
\bottomrule
\end{tabular}
\end{table}
...
corner bearing defining the single rotational degree of freedom, and encoders and other hardware. Note that the servo (brake system) is not included in the 2D prototype as its purpose is validating the control loop on a more well-understood problem. The 2D prototype is also a tool for calibrating and verifying the electronic circuit before jumping into a full 3D implementation. Motor calibration, electrical connections, and the telemetry system are all possible points of failure which are best tested in a smaller system first. Identifying errors in software or hardware is substantially easier with only one active wheel. Detailed geometry and the drawing set are in Appendix~\ref{app:cad_2d}. 
...
whose inertia is then tuned by \emph{ballast}: standard steel nuts
...
inertia and the ballast acts only as a trim, so that the count sits well
...
\midrule
$D_w$      & Ring outer diameter (capped by envelope) & \qty{13}{\centi\metre} \\
$w_r$      & Ring radial width                         & \qty{1.5}{\centi\metre} \\
$t_r$      & Ring axial thickness                      & \qty{1}{\centi\metre} \\
$n_s, w_s$ & Spoke count and width                     & $3$, \qty{1}{\centi\metre} \\
$\rho_p$   & Printed-material density                  & \qty{1290}{\kilo\gram\per\metre\cubed} \\
$r_b$      & Ballast radius                            & \qty{5.75}{\centi\metre} \\
$m_b$      & Net mass added per installed bolt         & \qty{2.5}{\gram} \\
$\Delta I_b$ & Net inertia added per installed bolt    & \qty{8.27e-6}{\kilogram\metre\squared} \\
$N$        & Bolt count reaching $I_{w,\text{target}}$ & \num{15} \\
$m_w$      & Resulting wheel mass                      & \qty{109}{\gram} \\
$I_w$      & Resulting wheel inertia                   & \qty{3.4e-4}{\kilogram\metre\squared} \\
\bottomrule
\end{tabular}
\end{table}
