...
Reaction wheels ($3\times$)   & 109 ($3\times$) & Sized (Sec.~\ref{sec:rw_sizing}) \\
Motors ($3\times$)            & 68 ($3\times$) & Drives reaction wheels \\
...
\midrule
\textbf{Total $M$}            & 1505 & \textbf{Closed by iteration} \\
\bottomrule
\end{tabular}
\end{table}
...
corner bearing defining the single rotational degree of freedom, and encoders and other hardware. Note that the servo (brake system) is not included in the 2D prototype as its purpose is validating the control loop on a more well-understood problem. Detailed geometry and the drawing set are in Appendix~\ref{app:cad_2d}. 
...
both removes plastic and adds steel at the largest available radius, so the net
inertia gain per station is large, and the target is reached by choosing how
many to install. This makes wheel inertia a discrete, reversible knob: the
...
material; both are checked in Section~\ref{sec:test_feasibility}, and a nyloc
...
\midrule
$D_w$      & Ring outer diameter (capped by envelope) & \qty{120}{\milli\metre} \\
$w_r$      & Ring radial width                         & \qty{12}{\milli\metre} \\
...
$r_b$      & Ballast pitch circle radius               & \qty{54}{\milli\metre} \\
$m_{\text{hw}}$ & Hardware mass per station (bolt $+$ nut) & \qty{9.0}{\gram} \\
$m_b$      & \emph{Net} mass added per station          & \qty{8.79}{\gram} \\
$\Delta I_b$ & \emph{Net} inertia added per station     & \qty{2.55e-5}{\kilogram\metre\squared} \\
$N$        & Station count reaching $I_{w,\text{target}}$ & \num{15} \\
$N_{\max}$ & Station count admitted by the gap rule    & \num{40} \\
$m_w$      & Resulting wheel mass                      & \qty{167}{\gram} \\
$I_w$      & Resulting wheel inertia                   & \qty{5.47e-4}{\kilogram\metre\squared} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Two geometric constraints bound $N$.} A station must leave sound
...
\paragraph{Sizing calculator and the iteration it supports.} The relations
...
One consequence of the objective is worth recording, because it runs against
intuition. Selection among admissible diameters is on minimum \emph{wheel
mass}, not minimum diameter: since $I \sim m r^2$, mass placed at a large
radius buys inertia far more cheaply than mass at a small one, so the smallest
wheel that ``works'' is the expensive one---it must make up its shortfall with
a large number of dense ballast stations, and that mass is mass the cube then
has to throw. The sweep shows the effect directly, with the smaller candidate
diameters requiring several times the ballast of the larger ones to reach the
same inertia.
