...
Each block of the capture is checked against its governing constraint before
bring-up: power entry and protection against the fuse rating and the inrush
into the driver bulk capacitance; the \qty{5}{\volt} logic rail against the
converter rating and the measured worst-case rail total (Section~\ref{sec:power_arch});
the CAN-FD bus against the linear-topology and termination rules of
Table~\ref{tab:signal_rules}; the encoder interfaces against the SPI cable
length limit and the \qty{3.3}{\milli\metre} air gap; the flight computer and
IMU against the sensor orientation and mounting location relative to the pivot
required by Section~\ref{sec:control_algorithms}; the telemetry link against
its transmit-burst current and return path off the logic rail; and the brake
servo drive against stall-current headroom on the logic rail, with its PWM
routed clear of the encoder SPI. The full constraint table is given in
Section~\ref{sec:schematics} (Table~\ref{tab:schematic_sheets}) and is not
repeated here.
...
(Table~\ref{tab:signal_rules}); the servo drives are sized on stall current.
...
Logic rail load, worst case    & \textcolor{red}{TBD} & Sec.~\ref{sec:power_arch} \\
