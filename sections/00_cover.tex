...
% ---------- Cover fields ----------
% These are aliases, not values: the project name, document type, programme,
% version and issue date come from the document-identity block in main.tex,
% which the running header and the PDF metadata read from as well. Edit them
% there, not here, so the cover and the header always agree (review comment
% CUBLI1-020). The date is \docdate rather than \today because the gate table of
% Sec. 2.3 reports gate status as at the date of issue, and that date must not
% change when the file is recompiled (review comment CUBLI1-015).
...
\begin{titlepage}
    ...
    \begin{minipage}{0.62\textwidth}
        ...
        \begin{tabular}{@{}p{0.42\linewidth}@{\hspace{1.4em}}l@{}}
            ...
            \covermetarow{Date of issue}{\coverDate}
        \end{tabular}
    \end{minipage}
