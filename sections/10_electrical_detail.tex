...
The seven sheets below are placeholders: each is numbered, captioned and listed
so that the document structure, the cross-references and the sheet index of
Table~\ref{tab:schematic_sheets} are already in place. Each caption states what
the sheet will show and the constraint it is checked against, so that a drawing
can be substituted without any other edit to this appendix. The complete
schematic set is reproduced at full page size in
Figure~\ref{fig:schematic_full}.

\subsubsection{Sheet 1 --- Power Entry and Protection}

% Pack connector, anti-spark inrush path, combined E-stop and fuse, split to
% motor bus and logic rail.
\begin{figure}[htbp]
\centering
...
% Buck converter, back-feed protection diode, rail bulk capacitance, per-device
% decoupling.
\begin{figure}[htbp]
\centering
...
% Linear bus chain across the three drivers and the flight computer, with split
...
Note: for the provided N35 $1/4" x 1/8"$ diametric disk magnet, the optimal distance in order to achieve the \qty{20}{\milli\tesla} -- \qty{80}{\milli\tesla} nominal magnetic field for the MA600 encoder is \qty{3.3}{\milli\metre}.

% SPI interface, one per axis, driver to encoder.
\begin{figure}[htbp]
\centering
...
% MCU, IMU interface, orientation relative to the body frame.
\begin{figure}[htbp]
\centering
...
% Telemetry module, serial interface to the MCU, supply decoupling.
\begin{figure}[htbp]
\centering
...
% Servo supply, decoupling and PWM routing.
\begin{figure}[htbp]
\centering
...
\subsubsection{Full Schematic Sheet}
\label{app:schematic_full}

The seven sheets above are also reproduced as a single drawing on the landscape
page that follows, at the largest size the format allows, so that the
interconnections between sheets can be followed without turning pages. The
landscape sheet is the reference copy; the per-sheet figures above are the
working views.
...
%
% To insert the real drawing, replace the \phfig line with, e.g.
%   \includegraphics[width=\linewidth,height=0.86\textwidth,keepaspectratio]{images/schematic_full.pdf}
% (on a landscape page \linewidth is the long edge; the height limit keeps a
% tall drawing inside the page). A multi-page PDF export can be repeated with
% \includegraphics[page=<n>]{...} inside further landscape blocks.
...
\centering
\phfig[0.80\textwidth]{0.96\linewidth}{Full electrical schematic ---
the complete sheet set of Table~\ref{tab:schematic_sheets} on one drawing:
power entry and protection, \qty{5}{\volt} logic rail, CAN-FD bus,
encoder interfaces, flight computer and IMU, telemetry link and brake servo
drive, with the inter-sheet nets shown.}
\caption[Full electrical schematic (landscape sheet)]{Full electrical schematic, reproduced at page size. Combines
sheets 1--7 (Figures~\ref{fig:sch_power_entry}--\ref{fig:sch_servo}) on a
single drawing so that the nets crossing between sheets can be traced
directly.}
