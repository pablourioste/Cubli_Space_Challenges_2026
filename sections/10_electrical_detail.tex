...
Figure~\ref{fig:schematic_full} is the schematic as currently captured: one flat
KiCad sheet carrying the whole system, reproduced on the landscape page that
follows at the largest size the format allows. It is the working drawing the
build is wired from, and it is the source the seven sheets above are split out
of --- the split is a release step, not a redraw, so the two must agree net for
net whenever a sheet is issued.
...
\centering
\includegraphics[width=\linewidth,height=0.98\textwidth,keepaspectratio]{images/schematic_full.pdf}
...
and the three brake servos. The per-sheet views of
Figures~\ref{fig:sch_power_entry}--\ref{fig:sch_servo} are drawn from this
capture as it is split for release.}
