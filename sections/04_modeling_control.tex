...
\ref{tab:jumpup_budget} gives the resulting target for the baseline cube,
which the reaction-wheel design in Section~\ref{sec:rw_sizing} must meet.
...
$\tau_g$           & Gravitational tip-over floor & $1.24$ N\,m \\
$\beta$            & Brake-amplification factor   & $1.33$ \\
$\omega_{\max}$    & Max wheel speed (design)     & $6000$ rpm ($628$ rad/s) \\
$h_w$              & Required angular momentum    & $0.264$ kg\,m$^2$/s \\
$I_{w,\text{target}}$ & \textbf{Target wheel inertia} & $\mathbf{4.20\times10^{-4}}$ \textbf{kg\,m}$^\mathbf{2}$ \\
\bottomrule
\end{tabular}
\end{table}
...
Section~\ref{sec:rw_sizing} meets with \qty{3.3}{\percent} margin. The
