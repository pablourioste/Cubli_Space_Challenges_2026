...
\section{Dynamic Modelling \& Control}
\label{sec:modelling}


The full Cubli is underactuated and simultaneously unstable about all three
axes when balanced on a corner (Section~\ref{sec:objectives}). The modelling,
identification, and control workflow is therefore first validated on a
single-face 1-DoF prototype, whose planar dynamics are the two-dimensional (2D)
model used throughout this section, before being generalized to the 3D cube in
...
%note from andrea not claude: a band brake thing could work if we see that the other teams are doing shit with their brakes
...
reaction wheels must stabilise the body about its edge and corner equilibria.
