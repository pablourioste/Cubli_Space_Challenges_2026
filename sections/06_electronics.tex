...
conductors --- Teensy 4.1, ESP32-C6 telemetry, BMI270 IMU, the three MG92B
brake servos and three MA600 encoders on the \qty{5}{\volt} logic rail, and the
three moteus-n1 drivers on the pack rail --- is populated from measurement
...
the sum of typical values. The \qty{5}{\volt} total is checked against the
LM2596 rating, which derates without a heatsink.
...
the connector and pin-assignment tables. Table~\ref{tab:schematic_sheets}
states, block by block, the constraint each part of that capture is checked
against before bring-up.

\begin{table}[htbp]
\centering
\caption[Electrical design constraints by block]{Electrical design constraints by block. Each is checked against the
capture of Figure~\ref{fig:schematic_full} before bring-up.}
\label{tab:schematic_sheets}
\small
\begin{tabular}{L{3.8cm} L{8.2cm}}
\toprule
Block & Governing constraint to verify \\
\midrule
Power entry and protection & E-stop, fuse rating, anti-spark inrush into the driver bulk capacitance \\
\qty{5}{\volt} logic rail & Converter rating vs.\ measured worst-case rail current; back-feed diode; rail bulk capacitance voltage rating \\
CAN-FD bus & Linear topology, stub length, split termination at the two physical ends only (Sec.~\ref{sec:bus_topology}) \\
Encoder interfaces ($3\times$) & SPI cable length and encoder air gap; driver placement follows from this, not the reverse \\
Flight computer and IMU & Sensor orientation and mounting location relative to the pivot (Sec.~\ref{sec:control_algorithms}) \\
Telemetry link & Transmit-burst current and its return path off the logic rail \\
Brake servo drive & Stall current headroom on the logic rail; PWM routing away from the encoder SPI \\
\bottomrule
\end{tabular}
\end{table}
