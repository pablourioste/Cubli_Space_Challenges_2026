...
The technical requirements in Table~\ref{tab:requirements} are categorized into functional behaviors, quantifiable performance targets, and hard design constraints. Verification methods are tracked independently of project milestones, ensuring that schedule adjustments do not compromise core requirements or invalidate safety margins. Comprehensive verification procedures and ongoing tracking are maintained in Appendix~\ref{app:verification}.

At present, baseline single-axis balance (PER-01, PER-03) and driver current bounds (PER-09) have been successfully verified on the 1-DoF test rig. Requirements involving multi-axis control, state-machine sequencing, and dynamic maneuvers remain open pending final assembly and testing of the integrated cube. 

To maintain sizing robustness, derived parameters such as wheel inertia (PER-06) are evaluated at the maximum allowable mass cap of \qty{1.8}{\kilo\gram} (CON-02), preserving operational margin for the current \qty{1.626}{\kilo\gram} build. Final verification of remaining open items—specifically full-speed wheel characterization and frame dimension reconciliation—will occur during final hardware integration.