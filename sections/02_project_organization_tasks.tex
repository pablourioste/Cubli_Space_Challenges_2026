...
The dates are the \emph{baseline} plan as locked at G1 and are left as
baselined, so that the slip against them stays visible; what was actually
achieved against each phase is the status column of
Table~\ref{tab:gates}.}
\label{tab:wbs_tasks}\\
\toprule
...
E2 & Rail, driver \& encoder bring-up & \qty{5}{\volt} rail soldered and trimmed to \qty{5.00}{\volt} under load; driver phase-wired and FOC-calibrated; MA600 encoder mounted at ${\le}\,\qty{1.5}{\milli\metre}$ air gap with verified read-back. Open-loop operation on the bench supply & Pablo & 1--3 Aug \\
...
E7 & Perfboard + battery power chain & Populate the distribution perfboard (two ground domains, one star point, split \qty{5}{\volt} rail); XT60 chain with anti-spark, fuse and ${\ge}\,\qty{50}{\volt}$ bulk capacitance; replicate the remaining actuation sets & Niccolo &8--11 Aug \\
